...
/*In this project, you have to perform the job of a sports analyst. You are given two datasets related
 to IPL (Indian Premier League) cricket matches. One dataset contains ball-by-ball data and the other 
...
/* 8)Create table ipl_ball02 with all the columns of the table ‘ipl_ball’ and an additional column 
ball_result containing values boundary, dot or other depending on the total_run (boundary for >= 4, dot 
for 0 and other for any other number) 
...
Pune Supergiant one with Rising Pune Supergiant and another one with Rising Pune Supergiants. Your task 
is to create a matches_corrected table with two additional columns team1_corr and team2_corr containing 
team names with replacing Rising Pune Supergiants with Rising Pune Supergiant. Now analyse these newly 
created columns.*/
...
/* 18)Create a new table ipl_ball04 with the first column as ball_id containing information of match_id, 
inning, over and ball separated by ‘-’ (For ex. 335982-1-0-1 match_id-inning-over-ball) and rest of the
 columns same as ipl_ball03). */
...
Create a temporary table with row numbers partitioned by team and ordered by total_runs in descending 
order. */
...
/* 21)Create table ipl_ball05 with all columns of ipl_ball04 and an additional column for row number 
partition over ball_id. (HINT : Syntax to add along with other columns,  row_number() over (partition by 
ball_id) as r_num).*/
...
As a sports analyst, successfully completed a comprehensive project on IPL cricket matches using SQL. 
The project involved two datasets: ball-by-ball data and match-wise data. Achievements and tasks 
...
4. **Data Retrieval:**
   ...
   - Retrieved data for matches where the result mode is 'runs' and the margin of victory is over 
   100 runs.
